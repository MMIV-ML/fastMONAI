...
# Path setup - models are relative to script location
SCRIPT_DIR = Path(__file__).parent

# Use CPU for inference
USE_CPU = True

# Runtime batching knob (not a PatchConfig field). Every patch/preprocessing
# parameter comes from the persisted patch config; see run_inference.
SW_BATCH_SIZE = 2
...
def save_series_pred(series_obj, save_dir, val='1234'):
    ...
    # Update study ID from patient ID
    ...
    for slice_idx in range(series_obj.slices):
        # Create unique UID per slice by modifying the base UID
...
def create_dicom_mask(pred, dicom_input_path, output_dir, uid_suffix, software_versions):
    ...
    """
    # Create a copy of the template series to hold the mask
    mask_obj = Series(str(dicom_input_path), opts={'slice_tolerance': 1e-2})

    # Overwrite SoftwareVersions (0018,1020) with the ensemble provenance passed in
    # (an ensemble tag, the per-fold run ids, and the fastMONAI version); any value
    # carried over from the source series is replaced.
    ...
    # Get mask data and transform to DICOM orientation
    new_mask = pred.numpy()
    new_mask = new_mask.squeeze()

    # Transform to DICOM orientation
    new_mask = np.transpose(new_mask, (-1, 1, 0))
    new_mask = new_mask.copy()  # Make contiguous
    new_mask = new_mask.astype(np.uint16)

    # Assign mask data to series using slice assignment
    mask_obj[:] = new_mask
...
def create_dicom_prob_mask(pred, dicom_input_path, output_dir, uid_suffix):
    ...
    """
    # Create a copy of the template series to hold the probability mask
    mask_obj = Series(str(dicom_input_path), opts={'slice_tolerance': 1e-2})

    # Get probability data and transform to DICOM orientation
    prob_data = pred.numpy().squeeze()

    # Transform to DICOM orientation
    prob_data = np.transpose(prob_data, (-1, 1, 0))
    prob_data = prob_data.copy()  # Make contiguous

    # Scale from [0,1] to [0,65535] for uint16
    prob_scaled = (prob_data * 65535).astype(np.uint16)

    # Assign scaled probability data to series
...
def run_inference(datafolder, output, model_type, use_tta=False):
    ...
    # Load the fold models first so a missing or empty models directory fails with an
    # actionable message (see load_models) before anything else runs. Passing the list
    # of models makes PatchInferenceEngine soft-vote them (per-patch probability
    # averaging before argmax). The engine also handles preprocessing (reorder/resample
    # via med_img_reader + Z-normalization), sliding-window patch sampling,
    # resize-to-original, and reorientation.
    models = load_models(config)

    # Load the full patch-inference config saved next to the models. This is the
    # single source of truth for preprocessing and patch settings (patch_size,
    # patch_overlap, aggregation_mode, apply_reorder, target_spacing,
    # keep_largest_component, normalization), so training and inference stay in
    # sync. PatchConfig(**config) reconstructs it; the engine reads apply_reorder,
    # target_spacing and normalization from the config.
    ...
    # Ensemble provenance for the mask DICOM (SoftwareVersions tag).
    ...
    # predict() reads the DICOM folder directly and returns class probabilities
    # in the original image space (resized + reoriented). The result is
    # [n_classes, *spatial], though the released reorientation path adds a leading
    # singleton ([1, n_classes, *spatial]); squeeze(0) normalizes both (it only
    # drops dim 0 when its size is 1, and n_classes here is 2). The binary mask is
    # derived via argmax + largest-connected-component, matching the validated CV path.
