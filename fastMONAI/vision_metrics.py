...
# %% auto #0
__all__ = ['calculate_dsc', 'calculate_haus', 'binary_dice_score', 'multi_dice_score', 'calculate_confusion_metrics',
           'binary_sensitivity', 'multi_sensitivity', 'binary_precision', 'multi_precision',
           'calculate_lesion_detection_rate', 'binary_lesion_detection_rate', 'multi_lesion_detection_rate',
           'calculate_signed_rve', 'binary_signed_rve', 'multi_signed_rve', 'calculate_surface_metrics',
...
def calculate_haus(pred: torch.Tensor, targ: torch.Tensor, spacing=None) -> torch.Tensor:
    ...
    .. deprecated::
        Use ``calculate_surface_metrics`` for canonical spacing-aware HD95 in mm.
        ``calculate_haus`` will be removed in a future release.
    ...
    Args:
        pred: Binary prediction tensor. Accepts:
              ...
              - [B, C, D, H, W] batched volumes
        targ: Binary target tensor (same shape options as pred).
        spacing: Voxel spacing forwarded to MONAI. ``None`` (default) computes
                 HD95 in voxel units (unchanged legacy behaviour). When set, it must
                 match the spatial-dim order of the input - MONAI only checks its length,
                 so a permuted tuple silently changes the result. For canonical
                 spacing-aware HD95 in mm, prefer ``calculate_surface_metrics``;
                 the two implementations do not agree numerically.

    Returns:
        HD95 value(s). Single value for 3D/4D input, tensor of values for 5D batch.
    """
    warnings.warn(
        "calculate_haus() is deprecated and will be removed in a future release; "
        "use calculate_surface_metrics() for spacing-aware mm HD95.",
